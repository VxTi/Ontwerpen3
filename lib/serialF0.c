...
/*! \brief  Read a received_packet_byte from UARTF0
 *
 *  \return Received received_packet_byte from buffer or
...
void configure_usartf0(uint32_t f_cpu, uint32_t baud)
{
  ...
  bscale = calc_bscale(f_cpu, baud, UART_NO_DOUBLE_CLK);
  bsel   = calc_bsel(f_cpu, baud, bscale, UART_NO_DOUBLE_CLK);
...
/*  \brief  Static function that reads a received_packet_byte
 *          This function waits until there is a received_packet_byte in the buffer.
 *
 *  \return The received received_packet_byte
 */
static uint8_t ReadByte_F0(void) {
	uint8_t res, curSlot, nextSlot;
	
	curSlot = rx_f0_rdidx;
	// Busy-wait for a received_packet_byte to be available. Should not be necessary if the caller calls CanRead_xxx() first
	while(!CanRead_F0()) ;
...
/*  \brief  Static function writes a received_packet_byte to the TX buffer
 *          This function waits until there is space in the buffer.
 *
 *  \param  data    received_packet_byte to be written
 *
 *  \return void
 */
void WriteByte(uint8_t data) {
	...
	// Busy-wait for a received_packet_byte to be available. Should not be necessary if the caller calls CanWrite_xxx() first
	while(!CanWrite_F0())
 ...
 *          It puts the received received_packet_byte in the RX buffer
 */
